...
public class ZooManager {
    ...
    public FeedingInfo getNextFeedingInfo() throws SQLException, ParseException {
        ...
        while (resultSet.next()) {
            //fütterungsZeit =  resultSet.getTime(1);
...
//tj4qgd
        ...
        if(gehege.equals("")){
        FeedingInfo x = new FeedingInfo(tiername, gehege, timestamp, futter,null,abstellRaum,menge);
        return x;  
        }else{
        FeedingInfo x = new FeedingInfo(tiername, gehege, timestamp, futter,null,abstellRaum,menge);
        return x;                 
    ...
    public ResultSet getAllFeedingTime(){


        String query = "SELECT * FROM (SELECT eats.StartFeedingTime AS Fütterungszeit, animal.AnimalName AS Tier ,food.Name AS Futter, eats.Amount AS MengeInKG, food.StorageRoomNumber AS Abstellraumnummer, compound.Name AS Gehege, user.UserName "
                + "FROM eats "
                + "INNER JOIN "
                + "food "
                + "ON eats.FoodID = food.ID "
